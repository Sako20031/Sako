...
const TOOLS = [
  {
    name: 'get_services',
    description: 'Возвращает список услуг компании с ценами и длительностью.',
...
function buildSystemPrompt(settings) {
  const name = settings.site_name || 'компания';
  const businessType = settings.business_type || 'сфера услуг с записью по времени';
  const address = settings.address ? `Адрес: ${settings.address}.` : '';
  const hours = settings.hours ? `Часы работы: ${settings.hours}.` : '';
  const notes = settings.assistant_notes ? `\nДополнительные правила от владельца бизнеса: ${settings.assistant_notes}` : '';
...
- Помогай клиенту выбрать свободное время и записаться на нужную услугу. Перед записью через create_booking всегда уточни дату, время, услугу и имя клиента, и убедись через check_availability, что слот свободен.
...
- Если не можешь помочь (например, вопрос не по теме компании), вежливо скажи об этом.${notes}`;
}
